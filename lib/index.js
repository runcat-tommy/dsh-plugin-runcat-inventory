/**
 * dsh-plugin-runcat-inventory —— Host 半端（逃猫-插件总览）
 *
 * 职责：
...
export function apply(ctx) {
  ...
  /** 把 profile 依赖声明转成人类可读的来源。 */
  function sourceOf(name, manifest) {
    const spec = manifest?.dependencies?.[name]
    if (typeof spec === 'string') {
      if (/^link:/.test(spec)) return '本地链接 (link)'
      if (/^file:/.test(spec)) return '本地路径 (file)'
      if (/^github:|^git\+/.test(spec)) return 'GitHub 安装'
      return 'npm 包 (' + spec + ')'
    }
    const bundles = manifest?.dsh?.profile?.bundles
    if (Array.isArray(bundles) && bundles.includes(name)) return '内置（随 DSH 安装）'
    return '其他'
  }
  ...
  function collectInventory() {
    ...
    for (const entry of ctx.loader.entries()) {
      ...
      rows.push({
        ...
        version: pkg?.version ?? '',
        source: sourceOf(entry.options.name, manifest),
        config,
      })
    }
    return rows
  ...
  async function setEnabled(patchId, name, enabled) {
    if (typeof patchId !== 'string' || patchId.length === 0 ||
      ...
      return { ok: false, error: '缺少 id 或 name' }
    }
    const profileDir = profileDirOf()
    if (profileDir === null) return { ok: false, error: '无法定位 profile 目录' }
    ...
    try {
      ...
      return { ok: false, error: '无法读取 ' + PATCH_FILENAME + '：' + String(error?.message ?? error) }
    }
    let yaml
    try {
    ...
    try {
      ...
      return { ok: false, error: '无法解析 ' + PATCH_FILENAME + '（可能含 !!js 表达式，请手动处理）：' + String(error?.message ?? error) }
    }
    if (!Array.isArray(patches)) return { ok: false, error: PATCH_FILENAME + ' 顶层必须是数组' }
    ...
    try {
      ...
      return { ok: false, error: '序列化失败：' + String(error?.message ?? error) }
    }
    try {
      await writeFile(patchPath, comment + body, 'utf8')
    } catch (error) {
      return { ok: false, error: '写入失败：' + String(error?.message ?? error) }
    }
