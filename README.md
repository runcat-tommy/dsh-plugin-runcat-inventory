...
> 版本：**v1.2.0** · 更新记录见 [CHANGELOG.md](CHANGELOG.md)（[English](CHANGELOG.en.md)）

一个更好用的 DSH 插件列表：**表格视图、状态过滤、启用/停用开关（热生效）、卸载、更新提醒、配置查看与复制、中英文界面自动切换**。
...
| 更新提醒 | 检测 npm registry 的 latest 版本（**分批扫描**：每批 4 个、批间 200ms、1 小时缓存）；本插件自身有新版本时在**面板最上方**显示醒目卡片（立即更新/暂时不用）；其他插件在版本列显示 `vX → vY` 高亮 + 行内"更新"按钮 + 顶部横幅；更新后标记"**重启生效**"；支持更新本插件自身 |
...
- **v1.2.0**（2026-08-27）：新增**更新提醒**——本插件自身更新在面板最上方
  醒目卡片展示（立即更新/暂时不用）；其他插件版本列 `vX → vY` 高亮 +
  行内"更新"按钮 + 顶部横幅；分批扫描（4/批、200ms 间隔、1h 缓存）；
  更新后标记"重启生效"；支持更新本插件自身。
